...
в течение до `CONFIG_UART_RADIO_PROBE_ATTEMPTS` попыток — поэтому рацию,
подключённую через несколько секунд после старта, тоже удастся обнаружить.
После этого задача сдаётся и мост остаётся отключённым (для повтора нужен
ребут). Скрестите TX↔RX и объедините общую землю.
